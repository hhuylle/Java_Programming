...
public class TestHobby {
    ...
    public static void main(String[] args) {
        ...
        // we can add both ways. Create object first and add, or create the object while adding
        ...
        // create 3 copies of arraylist's hobbies
        ArrayList<Hobby> outdoors = new ArrayList<>(hobbies);
        ArrayList<Hobby> requiresOthers = new ArrayList<>(hobbies);
        ArrayList<Hobby> highCost = new ArrayList<>(hobbies);

        outdoors.removeIf(p -> p.isOutdoors);
        System.out.println(outdoors);

        requiresOthers.removeIf(h -> h.requiresOthers );
        System.out.println(requiresOthers);

        highCost.removeIf(c -> c.annualCost>500);
        System.out.println(highCost);

        System.out.println("------------------------------------------------------");

        ArrayList<Hobby> lessThan200Indoors = new ArrayList<>(hobbies);
        lessThan200Indoors.removeIf(l -> l.annualCost>200 && l.isOutdoors);
